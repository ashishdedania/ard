...
use App\Http\Requests\Backend\Report\ScaredReportRequest;
use App\Repositories\Backend\Report\ReportRepository;
use App\Exports\Report\Scared\Export;
use App\Exports\InvoicesExport;

/**
 * ReportsController
 */
class ScaredController extends Controller {
     ...
     * @param ScaredReportRequest $request
     * @return type
     */
    public function index(ScaredReportRequest $request) {
        $client = getClients();
        return view('backend.reports.scared.index', compact('client'));
    }
     ...
     * @return type
     */
    public function getSession(ScaredReportRequest $request) {
        $getClientSession = $this->repository->getClientSession($request->clientId, $this->questionTypeId);
        $returnHTML = view('backend.reports.scared.session')
    ...
    public function fileName($clientId) {
        ...
        return 'SCARED-' . $clientName . '.xlsx';
    }

    /**
     * Export to excel for SCARED reports
     * @param ScaredReportRequest $request
     * @return type
     */
    public function generateExcel(ScaredReportRequest $request) {
