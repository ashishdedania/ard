...
namespace App\Repositories\Backend\KnowledgeBase;
...
class KnowledgebaseRepository extends BaseRepository {
	 ...
	 */
	const MODEL = Knowledgebase::class ;
	...
	public function getForDataTable() {
		return $this->query()
		            ->leftjoin(config('access.users_table'), config('module.knowledgebases.table').'.created_by', '=', config('access.users_table').'.id')
		            ->select([
				config('module.knowledgebases.table').'.id',
				...
				config('module.knowledgebases.table').'.rating',
				config('module.knowledgebases.table').'.average_rating',
				config('module.knowledgebases.table').'.status',
				config('module.knowledgebases.table').'.created_at',
				config('module.knowledgebases.table').'.created_by',
				config('module.knowledgebases.table').'.updated_at',
	...
	public function create(array $input) {
		...
		if ($knowledgebases->save()) {
			...
			if (!empty($input['file'])) {
				...
				Knowledgebase::where('id', $knowledgebasesId)->update(['file' => $filesNames]);
			}
		}
		return true;
