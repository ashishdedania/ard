...
class UserRepository extends BaseRepository {
	...
	public function getForDataTable($status = 1, $trashed = false) {
		...
		$dataTableQuery = $this->query()
		                       ...
		                       ->whereNotIn('role_user.role_id', [env('CLIENT_ROLE_ID')])
		                       ->select([
	...
	public function create($request) {
		...
		$input       = $request->except(['_token', 'assignees_roles', 'permissions', 'confirmation_email', 'password_confirmation', 'status', 'confirmed']);
		...
		DB::transaction(function () use ($user, $data, $roles, $permissions, $input) {
				if ($user->save()) {
					...
					if (isset($data['confirmation_email'])) {
						$url = \URL::to('/login');
						$input['login_link'] = '<a href='.$url.'>'.$url.'</a>';
						$options = [
							'data'                => $input,
