...
@section('content')
    @if (env('CLIENT_ROLE_ID') != 4)
    {{ Form::open(['route' => ['admin.access.user.change-password', $user], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'patch']) }}
    @else
        {{ Form::open(['route' => ['admin.client.update.password', $user], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'patch']) }}
    @endif
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('labels.backend.access.users.change_password', ['user' => $user->name]) }}</h3>
            </div><!-- /.box-header -->
